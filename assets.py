...
class Assets:
    def __init__(self):
        ...
        self.predictions = pd.read_csv("./Tools/predictions.csv")
        ...
        datePicker = dbc.Col(
            dcc.DatePickerSingle(
                id='time-select-day',
                min_date_allowed=dt(year=2020, month=8, day=25),   # self.timestamps[0]-pd.Timedelta('24H'),
                max_date_allowed=dt(year=2020, month=8, day=31),   # self.timestamps[-1],
                initial_visible_month=dt(year=2020, month=8, day=25),  # self.timestamps[0],
                date=dt(year=2020, month=8, day=25)  # self.timestamps[0].date()
            ),
            width=6
        )
        timePicker = dbc.Col(
            dcc.Dropdown(id='time-select-hour', options=self.hours, value=self.hours[0]),
            width=6
        )
